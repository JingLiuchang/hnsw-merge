...
for T in "${Ts[@]}"; do # 并行测试
  ...
  for db in "${datasets[@]}"; do
    ...
    EXECUTABLE="/home/jlc/hnsw-merge/cmake-build-debug/test_NSM_merge"
    ...
    MERGED_NSG_PATH="/mnt/ssd/merge_bench/${db}/random/bi-index-merged/${db}_random_NSM"

    ET=0
    RATIO=1.0

    # Log file to store outputs
    # LOG_FILE="/mnt/ssd/merge_bench/${db}/random/logs/NSM_merge.log"
  #  mkdir -p "/home/jlc/pg-fast-merging/performance/${db}/"
  #  LOG_FILE="/home/jlc/pg-fast-merging/performance/${db}/NSM_merge.log"
    mkdir -p "/home/jlc/hnsw-merge/logs/${db}/"
    LOG_FILE="/home/jlc/hnsw-merge/logs/${db}/NSM_merge.log"
    ...
    for param in "${PARAMS[@]}"; do
      ...
      echo "$EXECUTABLE $DATA_FILE $G $L $M ${sub_ef} ${sub_M} 2 $GRAPH_INDEX_FILE $OUTPUT_FILE $S $morder $T"
      $EXECUTABLE $DATA_FILE $G $L $M ${sub_ef} ${sub_M} 2 $GRAPH_INDEX_FILE $OUTPUT_FILE $S $morder $T 2>&1 | tee -a "$LOG_FILE"
    done
