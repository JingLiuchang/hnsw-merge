//
// Created for SIM (SimJoin MST-guided Sliding Merge) algorithm testing
//
...
int main(int argc, char** argv) {
    if (argc < 11 || argc > 13) {
        std::cout << argv[0]
                  << " data_file global_ef local_ef M sub_ef sub_M graph_num graph_index_file merged_nsg_path merge_order_selection [T] [merge_order_file]"
                  << std::endl;
        exit(-1);
    }
    ...
    std::string merge_order = std::string(argv[10]);
